...
export async function POST(req: Request) {
  ...
  if (op === 'suggest') {
    ...
    const system = [
      ...
      lessonPath
        ? '4c. LESSON-PATH MODE IS ON — this repository IS a guided path, so build every unit so a playable presentation can be generated from it. Under EACH unit/topic card, create AT LEAST TWO nested layers: (a) a nested "Objective" child card — its title is the skill/objective, its text explains in 1–2 sentences what the learner should understand or be able to DO after this unit; then (b) nested UNDER that Objective card, EXACTLY ONE LEAF card that is the SLIDE-BUILD PROMPT. Its "text" MUST BEGIN with the emoji "🔵 " and then read AS A DIRECTIVE TO THE PRESENTATION GENERATOR (e.g. "🔵 Build a lesson that evaluates …; have the learner practise …; check that they can …"), naming the concrete tasks, questions and skills to master this topic. This works for NON-ACADEMIC repositories too: for a catalogue/menu/service or portfolio repo, the Objective card states what the visitor should discover or decide (e.g. "know which plan fits their needs"), and the 🔵 prompt directs a SHOWCASE presentation of those items/services (what to present, compare and highlight) instead of a lesson. Produce ONE — and only one — slide-build-prompt card per objective; do NOT nest another prompt inside the prompt, and do NOT repeat it. The Objective card plus its single 🔵 prompt leaf are REQUIRED (two nested levels per unit) so the presentation tool knows exactly what to make.'
        : '',
      ...
      'ALSO NAME THE PLAN: include a top-level "title" and "subject". "title" is a concise, standard course/plan name in Title Case, 2–6 words, that names the SUBJECT (e.g. "Nutrition & Wellness", "Newtonian Mechanics", "Intro to Derivatives", "Italian for Beginners"). "subject" is the topic area in 1–4 words (e.g. "Nutrition", "Physics"). Derive BOTH from the ACTUAL CONTENT you just produced — the units/topics in THIS plan and the document — never from the user\'s instruction wording. NEVER echo a command ("Build a…", "Create a…", "Make a repo and slide tool about…", "A lesson that…"), and never make the title a whole sentence. If the goal is a bare instruction like "make a repo and slide tool about this course", ignore that phrasing entirely and name the plan after what the cards teach. Write them in the same language as the plan.',
