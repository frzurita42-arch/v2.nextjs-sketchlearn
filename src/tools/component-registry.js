...
const COMPONENT_REGISTRY = [
  ...
  item({ name: 'CoffeeMugLink', uses: 1, kind: 'container', createdAt: '2026-07-17T19:30:00Z',
    ...
    description: 'The ☕ "Buy me a coffee" donations link that sits in the RIGHT free quarter of a page banner (via PageHeader’s right slot). A hand-drawn coffee-mug SVG (shared DonationMug) over a small caption, the whole thing a link to a support page. Currently on the Repositories banner. Draws in the notebook sketch style so it matches the page rhythm. Shown on tablets & desktop only — hidden on phone screens (< 640px) so the centered title keeps the full width.',
  ...
  item({ name: 'PaperPlaneLink', uses: 1, kind: 'container', createdAt: '2026-07-17T19:30:00Z',
    ...
    description: 'The ✈ "Share / tell a friend" link that sits in the LEFT free quarter of a page banner (via PageHeader’s left slot). A hand-drawn paper-airplane SVG over a small caption; by default a mailto share so it works with no backend. Currently on the Repositories banner. A second banner-corner object tuned to the same sketch rhythm as the coffee mug. Shown on tablets & desktop only — hidden on phone screens (< 640px).',
