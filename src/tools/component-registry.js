...
const COMPONENT_REGISTRY = [
  ...
  item({ name: 'Coupon system', uses: 1, kind: 'component', createdAt: '2026-07-17T22:00:00Z',
    location: 'lib/coupon.ts + app/api/coupons + components/dashboard/TokenWindow.tsx (🎫 sections)',
    description: 'Admin-generated redeemable credit coupons. Admins pick a credit amount and generate a unique code (SKL-XXXX-XXXX) on a STANDARD minimalist ticket SVG — the credits & code are crisp SVG text, an optional Nano-Banana (Gemini) AI background is painted behind, and there’s a logo-upload slot. Any signed-in user redeems a code from their 🎫 token window to credit their wallet (and a plain user becomes a moderator once positive). Stored in the coupons table; each code redeems once.',
    inputs: 'Admin: credits number, ✨ AI-background toggle, 🖼 logo upload, Generate (POST /api/coupons) → shows the coupon image + code + copy/download. Redeem (all users): code field + Redeem (POST /api/coupons/redeem). GET /api/coupons lists created coupons.',
    recommendations: 'Add expiry dates and bulk-generate; composite the logo/code onto the AI raster for a single downloadable PNG.' }),
