...
- CRITICAL THINKING ALWAYS (this OVERRIDES the level — applies to Beginner just as much as Advanced): the question must demand reasoning, NOT recall. NEVER ask a "what is the definition of X" / "which of these is X" / "true or false" style recall question. Instead ask the learner to APPLY, ANALYZE, COMPARE, PREDICT, or JUSTIFY: give a short concrete scenario, a worked case, a "what happens if…", a cause-and-effect chain, or a "which conclusion follows from the slide" prompt, and make them reason from the slide's content to the answer. A lower level changes the VOCABULARY and the size of the reasoning step — simpler words, one inference instead of several — but it NEVER lowers the question to plain memorization. Even a beginner question must require the learner to think about the idea, not just remember a term. If your drafted question could be answered by someone who only memorized a definition without understanding it, REWRITE it into an application/analysis question before returning.
