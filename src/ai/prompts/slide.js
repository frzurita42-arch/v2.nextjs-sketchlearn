...
- COHESION & SMOOTH TRANSITIONS (the lesson must read as ONE continuous piece): every slide must advance the lesson's overall objective — mastering the topic being taught — and connect to its neighbours so moving from one subtopic to the next feels gradual and barely noticeable, never abrupt. Within the slide, the paragraphs build in order (introduce → develop → apply/consolidate) and never restate. OPEN with a short, natural BRIDGE from the previous slide's idea (a linking clause that carries the thread forward — e.g. "Now that we've seen X, we can look at how Y…"), and CLOSE by gently pointing toward what comes next, so consecutive slides share a visible through-line. Every slide should feel like the next logical step toward the objective, tied to what came before and what follows by a shared thread — never an isolated card dropped in from an unrelated angle. If this slide's concept sits far from the previous one, add an explicit bridging sentence that shows how they relate before developing the new idea.
...
function buildSlideUserPrompt({ topic, concept, level, slideNumber, totalSlides, historyText, branchText }) {
  return `Topic: ${topic}\nConcept being taught: ${concept}\nAudience level: ${level}\nThis is slide ${slideNumber} of ${totalSlides}.\nLESSON OBJECTIVE: every slide, including this one, exists to help the learner understand and master "${topic}". Keep this slide pointed at that objective — teach "${concept}" as one step toward it, not as a detour.\nSMOOTH TRANSITION: look at "Slides so far" below and OPEN this slide with a brief, natural bridge from the immediately previous slide's idea, then CLOSE by hinting at what comes next, so the move between subtopics feels seamless and the learner experiences one continuous lesson rather than separate cards.${slideNumber >= totalSlides ? ' This is the FINAL content slide: tie the threads together, wrap up the whole topic, and make the quiz a synthesis question that connects back to the objective.' : ''}\n${historyText}${branchText}`;
}
