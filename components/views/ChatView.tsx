            ...
            {messages.map((m, i) => {
              ...
              if (m.imageCredit && m.images?.length && m.polaroid) return (
                <div key={i} style={{ marginRight: 'auto', marginBottom: 14 }}>
                  <figure className="chat-polaroid" style={{ width: 90, padding: '4px 4px 0', margin: 0, transform: 'rotate(-2deg)', background: '#fffef9', border: '1px solid rgba(0,0,0,.08)', boxShadow: '1px 3px 7px rgba(40,26,8,.26)' }}>
                    ...
                    <figcaption style={{ fontSize: 10, textAlign: 'center', padding: '4px 2px 6px', lineHeight: 1.1 }}>{m.caption || 'our chat, so far'}</figcaption>
                  </figure>
                  <div style={{ marginTop: 4, fontSize: 9, color: 'var(--muted,#8a7f70)', maxWidth: 90 }}>🎨 Generated by {m.imageCredit}</div>
                </div>
              );
