            ...
            {messages.map((m, i) => {
              ...
              if (m.imageCredit && m.images?.length && m.polaroid) return (
                <div key={i} style={{ marginRight: 'auto', marginBottom: 14 }}>
                  <figure className="chat-polaroid">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={m.images[0]} alt="the coach's read of the chat" />
                    <figcaption>{m.caption || 'our chat, so far'}</figcaption>
                  </figure>
                  <div style={{ marginTop: 4, fontSize: 9.5, color: 'var(--muted,#8a7f70)', maxWidth: 96 }}>🎨 Generated by {m.imageCredit}</div>
                </div>
              );
