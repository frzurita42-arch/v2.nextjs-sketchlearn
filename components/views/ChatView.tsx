        ...
        <div className="chat-shell" style={{ flex: 1, minWidth: 0, height: '100%', padding: '0 16px', maxWidth: 880 }}>
          <div className="chat-log" id="chat-log" ref={logRef}>
            {messages.map((m, i) => {
              ...
              // A generated image. Coach "draw the chat" images come framed as a
              // Polaroid with a handwritten caption; a plain drawing shows bordered.
              if (m.imageCredit && m.images?.length && m.polaroid) return (
                <div key={i} style={{ marginRight: 'auto', marginBottom: 14 }}>
                  <figure className="chat-polaroid" style={{ width: 184, padding: '8px 8px 0', margin: 0, transform: 'rotate(-2deg)', background: '#fffef9', border: '1px solid rgba(0,0,0,.08)', boxShadow: '2px 4px 10px rgba(40,26,8,.26)' }}>
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={m.images[0]} alt="the coach's read of the chat" style={{ display: 'block', width: '100%', aspectRatio: '1 / 1', objectFit: 'cover' }} />
                    <figcaption style={{ fontSize: 15, textAlign: 'center', padding: '7px 3px 10px', lineHeight: 1.15 }}>{m.caption || 'our chat, so far'}</figcaption>
                  </figure>
                  <div style={{ marginTop: 4, fontSize: 10, color: 'var(--muted,#8a7f70)', maxWidth: 184 }}>🎨 Generated by {m.imageCredit}</div>
                </div>
              );
              if (m.imageCredit && m.images?.length) return (
                <div key={i} style={{ marginRight: 'auto', marginBottom: 12, maxWidth: 96 }}>
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={m.images[0]} alt="generated" style={{ width: '100%', borderRadius: 6, border: '1.5px solid var(--ink)' }} />
