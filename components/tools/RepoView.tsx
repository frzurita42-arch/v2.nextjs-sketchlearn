...
function RepoCollectionCard({ card, view, ctx, switchToRows, nested }: { card: RepoCard; view: 'grid' | 'row'; ctx: ViewCtx; switchToRows?: () => void; nested?: boolean }) {
  ...
  // Clip: a moderator (canPoster) edits; everyone else downloads/opens it.
  if (showClip) {
    if (canPoster) activeControls.push(<button key="clip" type="button" title={posterLinkIdx >= 0 ? 'Edit or delete the Moderator attachment' : 'Add a Moderator link or document'} style={attachIcon(posterLinkIdx >= 0)} onClick={eat(clipAction)}>📎</button>);
    else if (posterLinkIdx >= 0) activeControls.push(dlAnchor('clip', links[posterLinkIdx].url, '📎', 'Download / open the Moderator attachment'));
  }
  // Folder: the owner of the upload (canUser) edits their own; anyone else with a
  // copy just opens it.
  if (showFolder) {
    if (canUser) activeControls.push(<button key="folder" type="button" title={myUserLinkIdx >= 0 ? 'Edit or delete your document' : 'Upload your own document'} style={attachIcon(myUserLinkIdx >= 0)} onClick={eat(folderAction)}>📁</button>);
    else if (myUserLinkIdx >= 0) activeControls.push(dlAnchor('folder', links[myUserLinkIdx].url, '📁', 'Download / open the document'));
  }
