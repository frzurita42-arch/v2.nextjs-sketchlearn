...
function Field({ f, value, onChange, onSuggest, suggesting, fill }: {
  ...
  const labelStyle: CSSProperties = fill ? { width: '100%', margin: 0 } : { width: '100%', maxWidth: 240 };
  const ctrl = fill ? FILL_CONTROL : undefined;
  ...
  if (f.type === 'textarea') {
    return (
      <label className="field" style={{ ...labelStyle, gridColumn: '1 / -1' }}><span>{f.label}{suggestBtn}</span>
        <textarea value={value ?? ''} placeholder={f.placeholder} onChange={e => onChange(e.target.value)} style={{ ...ctrl, minHeight: 70 }} />
      </label>
    );
  }
  if (f.type === 'number') {
    return (
      <label className="field" style={labelStyle}><span>{f.label}{suggestBtn}</span>
        <input type="number" value={value ?? 0} placeholder={f.placeholder} onChange={e => onChange(e.target.value)} style={ctrl} />
      </label>
    );
  }
  if (f.type === 'toggle') {
    return (
      <label className="field" style={labelStyle}><span>{f.label}</span>
  ...
  if (f.type === 'date') {
    return (
      <label className="field" style={labelStyle}><span>{f.label}</span>
        <input type="date" value={value ?? ''} onChange={e => onChange(e.target.value)} style={ctrl} />
      </label>
    );
  }
  ...
  if (selectish) {
    return (
      <label className="field" style={labelStyle}><span>{f.label}
        ...
        {custom
          ? <input type="text" value={value ?? ''} placeholder={f.placeholder || 'Type your own…'} onChange={e => onChange(e.target.value)} style={ctrl} />
          : <select value={inList ? value : (f.options?.[0] ?? '')} onChange={e => onChange(e.target.value)} style={ctrl}>
  ...
  return (
    <label className="field" style={labelStyle}><span>{f.label}{suggestBtn}</span>
      <input type="text" value={value ?? ''} placeholder={f.placeholder} onChange={e => onChange(e.target.value)} style={ctrl} />
    </label>
  );
}

export function ToolFields({ fields, values, onChange, onSuggest, suggesting, single, fill }: {
  ...
  fill?: boolean;     // drop the 240px cap; controls fill the cell & match the manual settings fields
}) {
  if (!fields?.length) return null;
  return (
    <div className="settings-compact" style={single ? { gridTemplateColumns: 'minmax(0, 1fr)' } : undefined}>
      {fields.map(f => <Field key={f.id} f={f} value={values[f.id]} onChange={v => onChange(f.id, v)} fill={fill}
