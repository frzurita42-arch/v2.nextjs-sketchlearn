...
export function Collection<T>({
  ...
  searchPlaceholder = '🔍 search by name or @user', maxWidth = 900,
}: CollectionProps<T>) {
    ...
    <div>
      ...
      <div style={{ ...wrap, display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center', justifyContent: 'center', marginBottom: 8 }}>
        <input value={q} onChange={(e) => setQ(e.target.value)} placeholder={searchPlaceholder}
          style={{ fontSize: 13, flex: '1 1 200px', maxWidth: 280, padding: '5px 9px', borderRadius: 6, border: '1.5px solid var(--ink)' }} />
        {extra}
      ...
      {/* Pagination */}
      {perPage && pageCount > 1 && (
        <div style={{ ...wrap, display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 12, marginTop: 14 }}>
