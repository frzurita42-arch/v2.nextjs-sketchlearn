/* The country list for the sign-up form's searchable dropdown. Each entry is a
 * name + ISO-3166 alpha-2 code; the flag emoji is derived from the code (two
 * Regional Indicator letters), so we don't store an emoji per row. */

export interface Country { name: string; code: string }

// Turn a 2-letter country code into its flag emoji (🇺🇸 from "US"), by mapping
// each ASCII letter to its Regional Indicator Symbol codepoint.
...
export const COUNTRIES: Country[] = [
  ...
  { name: 'Congo (Kinshasa)', code: 'CD' }, { name: 'Costa Rica', code: 'CR' }, { name: 'Croatia', code: 'HR' },
