...
export function imageStyleDirective(style?: string): string {
  ...
  if (!s || s === 'Any') {
    return 'Art style: choose the style that best fits the subject for an ADULT / general audience — tasteful and mature, NOT a childish cartoon (unless the lesson is clearly meant for young children).';
  }
  const desc = STYLE_PROMPT[s] || s;
  const adult = s === 'Childish cartoon' ? '' : ' Keep it tasteful and suited to an adult / general audience.';
  return `Art style: render it as ${desc}.${adult}`;
}
